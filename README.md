# 🚀 Infra Lab

> Um laboratório pessoal para estudos, experimentos e documentação sobre Infraestrutura, DevOps, Cloud Computing, Automação e Observabilidade.

![GitHub last commit](https://img.shields.io/github/last-commit/SEU-USUARIO/infra-lab)
![GitHub Repo stars](https://img.shields.io/github/stars/Lcamargodasilva/infra-lab?style=social)
![License](https://img.shields.io/badge/license-MIT-blue.svg)

## 📖 Sobre o projeto

O **Infra Lab** é um ambiente de aprendizado contínuo criado para centralizar estudos, laboratórios práticos, documentações técnicas e automações relacionadas ao universo da Infraestrutura de TI.

O principal objetivo deste repositório é reunir conhecimento adquirido durante estudos, cursos, projetos pessoais e experiências práticas, documentando todo o processo de forma organizada.

Além de servir como uma base de consulta pessoal, este projeto também busca compartilhar conhecimento com a comunidade e demonstrar a evolução técnica ao longo do tempo.

---

# 🎯 Objetivos

- Estudar Infraestrutura de TI
- Aprender DevOps na prática
- Criar laboratórios reproduzíveis
- Documentar procedimentos técnicos
- Automatizar tarefas repetitivas
- Explorar ferramentas modernas
- Compartilhar conhecimento
- Desenvolver boas práticas de documentação
- Construir um portfólio técnico público

---

# 🛠️ Tecnologias e Ferramentas

Este laboratório aborda diversas tecnologias utilizadas em ambientes corporativos.

### Sistemas Operacionais

- Linux
- Windows Server
- WSL

### Infraestrutura

- Redes
- DNS
- DHCP
- Active Directory
- Virtualização

### Containers

- Docker
- Docker Compose
- Kubernetes

### Versionamento

- Git
- GitHub

### CI/CD

- Azure DevOps
- GitHub Actions

### Monitoramento

- Zabbix

### Observabilidade

- Graylog
- Prometheus
- Grafana

### Backup

- Bacula

### Cloud

- Microsoft Azure
- AWS

### Automação

- PowerShell
- Bash
- Python
- Terraform
- Ansible

---

# 📂 Estrutura do Projeto

```text
infra-lab/
│
├── README.md
├── ROADMAP.md
├── LICENSE
│
├── docs/
├── labs/
├── scripts/
├── docker/
├── assets/
└── .github/
```

---

# 🧪 Laboratórios

Cada laboratório possui sua própria documentação contendo:

- Objetivo
- Conceitos
- Instalação
- Configuração
- Exercícios
- Desafios
- Troubleshooting
- Referências

---

# 📚 Documentação

Além dos laboratórios, este repositório mantém uma documentação organizada contendo:

- Anotações de estudo
- Diagramas
- Troubleshooting
- Guias de instalação
- Procedimentos
- Boas práticas
- Checklists

---

# 🎯 Filosofia do Projeto

Este repositório prioriza o aprendizado prático.

Sempre que uma nova tecnologia é estudada, o objetivo é:

1. Entender os conceitos.
2. Implementar um laboratório funcional.
3. Documentar todo o processo.
4. Registrar problemas encontrados e suas soluções.
5. Compartilhar o conhecimento adquirido.

---

# 📈 Evolução Contínua

O Infra Lab está em constante evolução.

Novos laboratórios, documentações e automações serão adicionados conforme novos conhecimentos forem sendo adquiridos.

---

# 🤝 Contribuições

Sugestões, correções e melhorias são sempre bem-vindas.

Caso encontre algum problema ou tenha alguma ideia para melhorar este projeto, fique à vontade para abrir uma **Issue** ou enviar um **Pull Request**.

---

# 📄 Licença

Este projeto está licenciado sob a licença **MIT**.

Consulte o arquivo **LICENSE** para mais informações.

---

## ⭐ Se este projeto for útil para você

Considere deixar uma estrela no repositório. Isso ajuda a divulgar o projeto e incentiva a continuidade da documentação.